...
write.csv(tables_classif, "./cai_groups.csv" )
```
...
tables %>%
  group_by(genome) %>%
  summarise(cai = quantile(cai, 0.05)) %>%
  ggplot(aes(x = cai)) + geom_density()
```
...
tables %>%
  group_by(genome) %>%
  summarise(cai = quantile(cai, 0.95)) %>%
  ggplot(aes(x = cai)) + geom_density()
```
...
tables %>%
  group_by(genome) %>%
  summarise(n = sum(cai < quantile(cai, 0.05)) ) %>%
...
tables %>%
  group_by(genome) %>%
  summarise(n = sum(cai > quantile(cai, 0.95))) %>%
  ggplot(aes(x = n)) + geom_histogram(binwidth = 20)
```
